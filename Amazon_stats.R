...
library(purrr)
# Functions
cld <- function(posthoc){
  temp<-data.frame(Habitat=factor(c('RI','YPC','OPC','WPM','EPM','OPM','MOW','OSW'),levels=c('RI','YPC','OPC','WPM','EPM','OPM','MOW','OSW'),ordered=T),group=8:1)
  pval <- posthoc$p.adj
...
# BGC data
bgc <- read_excel(file.choose())
bgc$Habitat <- factor(bgc$Habitat,levels=c('RI','YPC','OPC','WPM','EPM','OPM','MOW','OSW'),ordered = TRUE)
bgc$Cruise[bgc$Cruise=='AT21-04'] <- 'AT2104'
bgc$Cruise <- factor(bgc$Cruise,levels=c('KN197','MV1110','AT2104','EN614','EN640','M174'),ordered = TRUE)
output <- "Habitat_defining_variable_stats.txt"

# Long format
def_var <- c('MLD','ChlMD','SST','SSS','NAI')
...
posthoc_cld <- list()
for (i in def_var){
  result<-gh_posthoc %>% filter(Def_variable==i) %>% cld()
...
posthoc_cld <- list()
for (i in def_var){
  result<- dunn %>% filter(Def_variable==i) %>% cld()
  posthoc_cld[[i]] <- result
}
posthoc_cld <- do.call(rbind, posthoc_cld)
rownames(posthoc_cld) <- NULL
